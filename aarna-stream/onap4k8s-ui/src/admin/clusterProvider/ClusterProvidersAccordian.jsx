...
export default function ControlledAccordions({ data, setData, ...props }) {
  ...
  const getAllNetworks = (providerRow, clusterIndex) => {
    if (data[providerRow].clusters && data[providerRow].clusters.length > 0) {
      if (clusterIndex === undefined) {
        data[providerRow].clusters.forEach((cluster) => {
          ...
          apiService
            ...
            .then((res) => {
              ...
              cluster.providerNetworks = res.spec["provider-networks"];
              cluster.networksStaus = res.spec.status;
            })
            .catch((err) => {
        ...
        let request = {
          providerName: data[providerRow].metadata.name,
          clusterName: data[providerRow].clusters[clusterIndex].metadata.name,
        };
        apiService
          .getAllClusterNetworks(request)
          .then((res) => {
            data[providerRow].clusters[clusterIndex].networks =
              res.spec.networks;
            data[providerRow].clusters[clusterIndex].providerNetworks =
              res.spec["provider-networks"];
            data[providerRow].clusters[clusterIndex].networksStaus =
              res.spec.status;
          })
          .catch((err) => {
            console.log("error getting cluster networks : ", err);
  ...
  const handleSubmit = (values, setSubmitting) => {
    ...
    apiService
      ...
      .then((res) => {
        ...
        getAllNetworks(
          selectedRowIndex,
          data[selectedRowIndex].clusters.length - 1
        );
